...
public class Transaction {
    ...
    public void processTransaction(Payment payment) {
        ...
        if (!this.payment.validatePayment()) {
            ...
            this.payment = new Payment(this.amountDue, 0, new PaymentMethod(PaymentMethod.CASH));
        }

        if (customer != null && customer.isMember()) {
            if (this.payment != null && this.payment.getAmountPaid() >= this.amountDue) {
                this.earnedPoints = customer.computeEarnedPoints(this.amountDue);
        } else {
            this.earnedPoints = 0; // No points if payment is insufficient
        }
        }
    }
