...
public class Reciept{
    ...
    public Reciept(String receiptID, Transaction transaction){
    ...
    public String toDevice(){
        ...
        if(transaction.getPointsRedeemed() > 0){
            text += String.format("Points Redeemed : -%.2f (%d points)%n", transaction.getPointsRedeemed(), (int)Math.round(transaction.getPointsRedeemed()));
        }
        ...
        if(transaction.getPayment() != null){
            text += String.format("PAID   : %.2f (%s) %n", transaction.getPayment().getAmountPaid(),transaction.getPayment().getMethod().getModeOfPayment());
